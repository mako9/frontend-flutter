...
class ItemDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) =>
          ItemDetailCubit.ofInitialState(item: _initialItem),
  ...
  Widget build(BuildContext context) {
    ...
    return PlatformScaffold(
      appBar: PlatformAppBar(
        title: PlatformText(AppLocalizations.of(context)!.itemDetailScreen_title),
      ),
      body: Container(
        padding: const EdgeInsets.all(30.0),
        alignment: Alignment.topCenter,
        child: BlocBuilder<ItemDetailCubit,
            DataResponse<Item>>(builder: (_, dataResponse) {
                ...
                Container(
                  ...
                  child: Column(children: [
                    ...
                    Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        PlatformText('${AppLocalizations.of(context)!.description}:'),
                        const SizedBox(width: 32),
                        Expanded(child:
                          PlatformText(item?.description ?? '-', overflow: TextOverflow.visible)
                        ),
                      ],
                    ),
                  ]),
